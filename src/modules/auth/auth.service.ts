...
export const authService = {
  async register(input: { name: string; email: string; password: string }, meta: { userAgent?: string; ip?: string }) {
    const existing = await authRepo.findUserByEmail(input.email);
    if (existing) throw new HttpError(409, "Email already registered");

    const passwordHash = await hashPassword(input.password);
    const user = await authRepo.createUser({ name: input.name, email: input.email, passwordHash });

    const session = await authRepo.createSession({ userId: user.id, userAgent: meta.userAgent, ip: meta.ip });

    // create email verify token (mock link)
    const raw = generateRawToken();
    const tokenHash = hashToken(raw);
    const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000);
    await authRepo.createToken({ userId: user.id, type: TokenType.EMAIL_VERIFY, tokenHash, expiresAt });
  ...
  async verifyEmail(rawToken: string) {
    ...
    const tokenRow = await authRepo.findValidToken({ tokenHash, type: TokenType.EMAIL_VERIFY });
    if (!tokenRow) throw new HttpError(400, "Invalid or expired verification token");
  ...
  async forgotPassword(email: string) {
    ...
    await authRepo.createToken({ userId: user.id, type: TokenType.PASSWORD_RESET, tokenHash, expiresAt });
  ...
  async resetPassword(rawToken: string, newPassword: string) {
    const tokenHash = hashToken(rawToken);
    const tokenRow = await authRepo.findValidToken({ tokenHash, type: TokenType.PASSWORD_RESET });
    if (!tokenRow) throw new HttpError(400, "Invalid or expired reset token");

    const user = await authRepo.findUserById(tokenRow.userId);
    if (!user || user.isArchived) throw new HttpError(400, "Invalid reset request");
    if (user.role === "ADMIN") throw new HttpError(403, "Admin password reset disabled");
